							...
							<div class="panel-body">
								<form id="UpdateData" method="POST">
									<div class="table-wrap">
											...
											<table  class="table table-hover display  pb-30" >
												...
												<tbody>
													...
													while ($Data = mysqli_fetch_array($Siswa)) {
														$DataMapel = mysqli_fetch_array(mysqli_query($conn,"select * from tb_nilai where id_siswa='".$Data['id_siswa']."' and tanggal='".$tanggal."' and id_mata_pelajaran='".$Mapel."'"));
														?>
														<tr>
															<td><?=$no++;?></td>
															<td>
																...
																<?php if ($DataMapel['action_update'] == 2) {?>
																	<td>
																		<input type="number" class="form-control" name="nominal[<?=$i;?>]" id="nominal" value="<?=$Data['id_siswa'] == $DataMapel['id_siswa'] ? ''.$DataMapel['nilai'].'' : 'Tidak Diketahui';?>" readonly/>
																	</td>
																<?php }else{?>
																	<td>
																		<input type="number" class="form-control" name="nominal[<?=$i;?>]" id="nominal" value="<?=$Data['id_siswa'] == $DataMapel['id_siswa'] ? ''.$DataMapel['nilai'].'' : 'Tidak Diketahui';?>" required="">
										...
										<br>
										<?php 
          $DataMapelNya = mysqli_fetch_array(mysqli_query($conn,"select * from tb_nilai where  tanggal='".$tanggal."' and id_mata_pelajaran='".$Mapel."'"));
     ...
     swal({
      ...
      if (isConfirm) {
       $.ajax({
        type: 'POST',
        data: UpdateData,
        url: 'App/Page/Nilai/simpan.php?Update',
        dataType: "JSON",
        cache:"false",
     ...
     swal({
      ...
      if (isConfirm) {
       $.ajax({
        type: 'POST',
        data: UpdateData,
        url: 'App/Page/Nilai/simpan.php?Valid',
        dataType: "JSON",
        cache:"false",
        success: function(respone) {
         if (respone.status == 'success') {
