 	...
 	<div class="container-fluid">
 						...
 						<div class="panel-body">
 							<div class="form-wrap" id="Form">
 								...
 								if (isset($_GET['Form'])) {
 									...
 									$Keterangan = mysqli_fetch_array(mysqli_query($conn,"select id,periode from tb_periode where id='".$Data."'"));
 									?>
 									<form class="form-horizontal" id="FormEdit" method="POST">
 									<?php }else{?>
 										<form class="form-horizontal" id="FormTambah" method="POST">
 										<?php }?>
 										<label class="control-label " for="email_hr">PERIODE:</label>
 										<div class="form-group">
 											<div class="col-sm-12">
 												<?php if(isset($_GET['Form'])){?>
 													<input type="hidden" value="<?=$Keterangan['id'];?>" class="form-control" id="id" name="id">
 												<?php }?>
 												<input type="text" value="<?php if(isset($_GET['Form'])){echo"".$Keterangan['periode']."";}else{echo"";}?>" class="form-control" id="nama" name="nama" placeholder="2019 - 2020">
 												<?php if(isset($_GET['Form'])){?>
 										...
 										<div class="form-group mb-0">
 											<div class="col-sm-offset-0 col-sm-10">
 												...
 												<a href="?Halaman=Periode" class="btn btn-primary btn-anim"><i class="ti-angle-double-left"></i><span class="btn-text" title="Kembali">Kembali</span></a>
 											</div>
 										</div>
 									</form>
 								</div>
 		...
 		<script>
 			$('#FormTambah').on('submit',function(e) {
 				...
 				var FormTambah = $(this);
 				$.ajax({
 					url: 'App/Page/Periode/simpan.php?Kirim',
 					type: "POST",
 			...
 			$('#FormEdit').on('submit',function(e) {
 				...
 				swal({
 					...
 					text: "Ingin Mengubah Periode ini Jika Dihapus Akan Berpengaruh Pada Data Lainnya..!",
 					icon: "warning",
 					...
 					if (isConfirm) {
 						$.ajax({
 							...
 							url: 'App/Page/Periode/simpan.php?Update',
 							dataType: "JSON",
 							cache :"false",
 							success: function (respone) {
 								if (respone.status == 'success') {
